...
for h in range(len(ddd)):
    ...
    ax[0].loglog(k1[h]*d_i,fek1[h]/fm1,linestyle=l[h],linewidth='0.5',color=c[h],label=str(m))
    ax[0].axvline(di[h], color=c[h], linestyle=':')
    ax[1].loglog(k2[h]*d_i,fek2[h]/fm2,linestyle=l[h],linewidth='0.5',color=c[h])
    ax[1].axvline(di[h], color=c[h], linestyle=':')
    ax[2].loglog(k3[h]*d_i,fek3[h]/fm3,linestyle=l[h], linewidth='0.5',color=c[h])
    ax[2].axvline(di[h], color=c[h], linestyle=':')

#labels
ax[0].text(0.3,0.2,r'$\epsilon^{B}(k)$')
#ax[0].set_title(r'- 25  -.- 50  -- 100')
#ax[0].set_title(r'Omni-Spectra at $\tau=4.9\tau_{0}$')

ax[1].text(0.3,0.2,r'$\epsilon^{ve}(k)$')

ax[2].text(0.3,0.2,r'$\epsilon^{vi}(k)$')
ax[2].set_xlabel(r'$k d_i$')

# Put a legend below current axis
ax[0].legend(loc='upper center', bbox_to_anchor=(0.5, 1.05), ncol=7)
